...
def plot_network(nodes: List[NODE_TYPE],
    ...
    node_hovers = {nID: node_hovers[nID]
                   if nID in node_hovers else None for nID in nodes}

    # Init node labels, if required.
    node_labels = {nID: node_labels[nID]
                   if nID in node_labels else str(nID) for nID in nodes}
    ...
    if not edges:
        ...
        edge_weights = {eID: float(edge_weights[eID])
                        if eID in edge_weights else wL for eID in edges}

        if max(edge_weights.values()) > wU:
            edge_weights = {eID: edge_weights[eID] / max(edge_weights.values()) * wU
                            for eID in edges}
        if min(edge_weights.values()) < wL:
            edge_weights = {
                eID: wU - (wU - edge_weights[eID]) / (wU - min(edge_weights.values())) * (wU - wL)
                for eID in edges}
        ...
        if edge_hovers is None:
            edge_hovers = {}
        edge_hovers = {eID: edge_hovers[eID]
                       if eID in edge_hovers else None for eID in edges}
...
def plot_orderbook(t1: str,
    ...
    fig = plotly.subplots.make_subplots(
        rows=2, cols=2,
        subplot_titles=("buy/sell amount in %s" % t1, "buy/sell amount in %s" % t2,
                        "net amount in %s" % t1, "net amount in %s" % t2),
    ...
    fig['layout'].update(
        height=1000, width=1000, paper_bgcolor='white', plot_bgcolor='white', title=title)
