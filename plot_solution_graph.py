...
def generate_plot(
    ...
    trading_volume_tokenpairs = {
        (t1, t2): (tokenpair_amounts_sold.get((t1, t2), 0) * token_prices.get(t1, 0)
    ...
    xrates = {
        ...
        for t1 in tokens for t2 in tokens
    ...
    node_hovers = {t: "=== %s ===<br>clearing price : %s<br>sold : %s<br>bought : %s"
                   % (t,
    ...
    edge_hovers = {(t1, t2): "=== %s/%s ===<br>"
                   ...
                   % (t1, t2,
                      util.decimal_to_str(xrates.get((t1, t2))), t2, t1,
    ...
    for idx, o in enumerate(inst['orders']):
        if o['execSellAmount'] > 0:
            ...
            logging.info(
                ...
                % (idx, aID[:6],
                   util.decimal_to_str(xS_exec), util.get_token_name(tS),
                   util.decimal_to_str(xB_exec), util.get_token_name(tB)))
